...
class BookKeywordsFileSaveToDBOperator(BaseOperator):
    ...
    def execute(self, context: Context):
        ...
        # SQL 쿼리 로드
        sql_path = '/opt/airflow/sql/insert_book_keywords.sql'
        ...
        for file_path in json_files:
            ...
            # ISBN13 추출
            isbn13 = response['additionalItem']['isbn13']
        ...
        # DB에 저장
        if all_keywords:
            cursor.executemany(insert_sql, all_keywords)
            db.commit()
            print(f"DB 저장 완료: {len(all_keywords)}건")
